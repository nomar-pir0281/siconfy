...
export const HistorialPage: React.FC = () => {
  ...
  const handleGoToTool = (tipo: string) => {
    // Mapeo simple de los tipos de cálculo a las rutas
    const tabMap: Record<string, string> = {
      'planilla': '/planilla',
      'liquidacion': '/liquidacion',
      'indemnizacion': '/indemnizacion',
      'vacaciones': '/vacaciones',
      'aguinaldo': '/nomina', // assuming aguinaldo is under nomina
      'salario': '/nomina'
    };

    const targetTab = tabMap[tipo.toLowerCase()] || '/';
    navigate(targetTab);
  };

  // --- NUEVA FUNCIÓN: IMPRIMIR ---
  const handlePrint = () => {
    if (selectedCalculation?.tipo === 'planilla' && Array.isArray(selectedCalculation.detalles)) {
    ...
    <div className="max-w-6xl mx-auto p-4 animate-fade-in print:p-0 print:max-w-none">
      ...
      <style>{`
        @media print {
            ...
            .print-visible-modal, .print-visible-modal * {
                visibility: visible;
            }
            .print-visible-modal {
                position: absolute;
              ...
              {calculations.map((calc, index) => (
                <tr key={index} className="hover:bg-blue-50 transition-colors cursor-pointer" onClick={() => handleViewDetails(calc)}>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${calc.tipo === 'planilla' ? 'bg-purple-100 text-purple-800' : 'bg-orange-100 text-orange-800'
                      }`}>
                      {calc.tipo}
                    </span>
                  </td>
            ...
            <div className="p-6 print:p-0">
              ...
              <div className="mt-8 flex justify-between items-center pt-4 border-t border-gray-100 no-print">
                <button
                  ...
                  className="flex items-center gap-2 text-blue-600 font-bold hover:bg-blue-50 px-3 py-1.5 rounded transition text-xs"
                ...
                <div className="flex gap-3">
                  <button
                    onClick={handlePrint}
                    className="flex items-center gap-2 bg-gray-700 text-white px-3 py-1.5 rounded-lg font-bold hover:bg-gray-800 transition shadow-sm text-xs"
                  >
                    <span>🖨️</span> {selectedCalculation.tipo === 'planilla' && Array.isArray(selectedCalculation.detalles) ? 'Ver Reporte' : 'Imprimir'}
                  </button>
                  <button
                    onClick={handleCloseModal}
                    className="px-3 py-1.5 bg-gray-200 text-gray-800 font-bold rounded-lg hover:bg-gray-300 transition text-xs"
                  >
                    Cerrar
                  </button>
                </div>
              ...
              {/* Footer solo para Impresión (Legacy fallback) */}
      ...
      {/* MODAL DE VISTA PREVIA (REPORTES FULL) */}
      <ReportPreviewModal
        isOpen={showReportPreview}
        onClose={() => setShowReportPreview(false)}
        title={selectedCalculation?.tipo.toUpperCase() === 'PLANILLA' ? "PLANILLA DE PAGO (HISTÓRICO)" : "REPORTE HISTÓRICO"}
        subtitle={`Generado el: ${selectedCalculation ? formatDate(selectedCalculation.fecha) : ''}`}
        orientation="landscape"
      >
        {selectedCalculation?.tipo === 'planilla' && Array.isArray(selectedCalculation.detalles) ? (
          <PlanillaReportTable data={selectedCalculation.detalles} />
        ) : (
          <div className="p-4 text-center text-gray-500">
            No hay detalles visualizables para este reporte.
          </div>
        )}
      </ReportPreviewModal>
    </div>
